...
#_*_coding:utf-8_*_
"""
很多程序都有记录日志的需求，并且日志中包含的信息即有正常的程序访问日志，还可能有错误、警告等信息输出，python的logging模块提供了标准的日
志接口，你可以通过它存储各种格式的日志，logging的日志可以分为 debug(), info(), warning(), error() and critical()5个级别，下面我们看一下怎么用。
...
# logging.basicConfig(filename='log_test.log',level=logging.INFO)   #INFO 级别高于DEBUG
# logging.basicConfig(filename='log_test.log',level=logging.DEBUG)
logging.basicConfig(filename='log_test.log',
                    level=logging.DEBUG,
                    format='%(asctime)s %(levelname)s %(pathname)s  %(filename)s  %(relativeCreated)d  %(message)s ',
                    datefmt='%m/%d/%Y %I:%M:%S %p'
                    )
def sayhi():
    logging.error("from savhii ....")
sayhi()
logging.debug("This  massage should go  to the log file")
...
#console  DEBUG
#global INFO   default  level  warning
#file Warning
# 全局设置为DEBUG后  console handler 设置为INFO  如果输出的日志级别是debug，那就不会在屏幕输出
